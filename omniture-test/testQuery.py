...
class QueryTest(unittest.TestCase):
    ...
    def test_report_run(self):
        self.assertIsInstance(self.analytics.suites[test_suite].report.run(), omniture.Report,
                              "The run method doesn't work to create a report")
    ...
    def test_element_paratmers(self):
        ...
        This isn't a conclusive test. I really should run to two reports and compare the results to make sure it is
        corrent. However, these tests need to be able run on any report suite and some reports suites (like ones that
        are currenly being used) don't have 10 items in the page name
        """
    ...
    def test_anomoly_detection(self):
        basic_report = self.analytics.suites[test_suite].report.metric("pageviews")\
            .range('2014-05-1', '2014-05-07').anomaly_detection()
        queue = []
        queue.append(basic_report)
        response = omniture.sync(queue)
    ...
    def test_sortBy(self):
            ...
            basic_report = self.analytics.suites[test_suite].report.element('page')\
                .metric('pageviews').metric('visits').sortBy('visits')
    ...
    def test_current_data(self):
        ...
        basic_report = self.analytics.suites[test_suite].report.element('page')\
            .metric('pageviews').metric('visits').currentData()
        self.assertEqual(basic_report.raw['currentData'], True)
        
    def test_inline_segments(self):
        """ Make sure inline segments work """
        report = self.analytics.suites[test_suite].report.element('page').metric('pageviews')\
            .metric('visits').filter(element='page', selected=["test", "test1"])
        self.assertEqual(report.raw['segments'][0]['element'], "page", "The inline segment element isn't getting set")
        self.assertEqual(report.raw['segments'][0]['selected'], ["test", "test1"],
                         "The inline segment selected field isn't getting set")

    def test_hour_granularity(self):
        """ Make sure granularity works """
        report = self.analytics.suites[test_suite].report.granularity('hour')
        self.assertEqual(report.raw['dateGranularity'], "hour",
                         "Hourly granularity can't be set via the granularity method")
   
    def test_day_granularity(self):
        """ Make sure granularity works """
        report = self.analytics.suites[test_suite].report.granularity('day')
        self.assertEqual(report.raw['dateGranularity'], "day",
                         "daily granularity can't be set via the granularity method")
 
    def test_week_granularity(self):
        """ Make sure granularity works """
        report = self.analytics.suites[test_suite].report.granularity('day')
        self.assertEqual(report.raw['dateGranularity'], "day",
                         "Weekly granularity can't be set via the granularity method")

    def test_quarter_granularity(self):
        """ Make sure granularity works """
        report = self.analytics.suites[test_suite].report.granularity('quarter')
        self.assertEqual(report.raw['dateGranularity'], "quarter",
                         "Quarterly granularity can't be set via the granularity method")
        
    def test_year_granularity(self):
        """ Make sure granularity works """
        report = self.analytics.suites[test_suite].report.granularity('year')
        self.assertEqual(report.raw['dateGranularity'], "year",
                         "Yearly granularity can't be set via the granularity method")
